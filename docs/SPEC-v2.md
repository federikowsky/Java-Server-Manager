...
| `jsm.templates.manage` | Manage Templates | `$(settings-gear)` | view/title@3 | `view == javaServerManagerView` | Open template management UI | mandatory |
...
| D-14 | **SPEC-v2 as single source of truth** | (A) Keep three spec files and reconcile manually, (B) Create one unified SPEC-v2.md that supersedes all previous specs | (B) Unified SPEC-v2 | Avoids spec drift. One normative document means no ambiguity during implementation. Old specs preserved read-only for archaeology. |
| D-15 | **Big bang rewrite** | (A) Incremental migration preserving current tests, (B) Full rewrite from SPEC-v2 | (B) Big bang | Current codebase has too many structural issues (singletons, layer violations, flat config, no DI, monolithic commands). Incremental migration would require maintaining two parallel architectures. New code starts clean from the spec. |
