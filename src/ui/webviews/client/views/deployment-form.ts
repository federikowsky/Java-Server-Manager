/**
 * Deployment form view — page-level overrides/extensions for the deployment form.
 */
...
/** Initialize the deployment form view from an `init` message. */
export function initDeploymentForm(msg: Extract<HostToWebview, { command: 'init' }>): void {
...
/** Handle host messages for the deployment form. */
export function handleDeploymentFormMessage(msg: HostToWebview): void {
  switch (msg.command) {
    case 'loaded':
      populateForm(msg.data);
      break;
...
function addFormButtons(root: HTMLElement, mode: 'create' | 'edit'): void {
  ...
  submitBtn.textContent = mode === 'create' ? 'Add Deployment' : 'Save Changes';
